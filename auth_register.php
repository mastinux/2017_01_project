<?php
    ...
    switch($_SERVER['REQUEST_METHOD']) {
        ...
        case 'POST':{
            if ( !isset($_POST['name']) || !isset($_POST['surname']) || !isset($_POST['email']) || !isset($_POST['password']) || !isset($_POST['password-repeated']) )
                redirect_with_message("auth_login.php", "w", "Name, surname, email or password not set in registration form.");
            $name = $_POST['name'];
    ...
    if ( $name == "" || $surname == "" || $email == "" || $password == "" || $password_repeated == "" ) {
        // request does not contain email or password or password_repeated
        redirect_with_message("auth_login.php", "w", "Name, surname, email or password not inserted in registration form.");
    }
    else{
